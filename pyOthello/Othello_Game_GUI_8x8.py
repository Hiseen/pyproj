...
class Othello_TK:
    def __init__(self):
        ...
        print("loading MCT AI...")
        t1=time.time()
        self.AI=MCT_Node.load()
        self.AI_root=self.AI
        t2=time.time()
        print("finished loading...{}".format(t2-t1))
        self.__init_GUI()
        mainloop()
    ...
    def __init_GUI(self):
        ...
        self.widget_dict['rows_scale']=Scale(self.window,from_ = 8,to = 8,resolution = 2,orient = HORIZONTAL,variable=NUM_OF_ROWS)
        
        NUM_OF_COLS=IntVar()
        NUM_OF_COLS.set(4)
        self.widget_dict['cols_label']=Label(self.window, text='number of columns of the board', font=DEFAULT_FONT)
        self.widget_dict['cols_scale']=Scale(self.window,from_ = 8,to = 8,resolution = 2,orient = HORIZONTAL,variable=NUM_OF_COLS)
    ...
    def __AI_moves(self):
        ...
        pos=self.AI.MCTS(self.Game)
        self.Game.drop(pos)
        self.AI=self.AI.get_next(pos)
    ...
    def __GameStart(self):
        ...
        if self.Game:
            del self.Game
        self.AI=self.AI_root
        self.Game=Othello_Game(tuple(map(lambda x:self.data_dict[x].get(),['rows','cols','fm','tf','wd'])),True)
        self.data_dict['winner'].set('')
        self.data_dict['stats'].set('BLACK:2 WHITE:2 TURN:{}'.format(self.data_dict['fm'].get()))
